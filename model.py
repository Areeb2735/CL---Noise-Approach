...
from utils.adapter_cl_net import CNN_Adapter_Net_CIL_V2
from utils.finetune_il import Finetune_IL
from utils.toolkits import count_parameters, tensor2numpy
    ...
    def prepare_model(self, checkpoint=None):
        if self._network == None:
            self._network = CNN_Adapter_Net_CIL_V2(self._logger, self._config.backbone, self._config.pretrained,
                    pretrain_path=self._config.pretrain_path, layer_names=self._config.layer_names, task_id = self._config.task_id)
        self._network.task_id_update()
        # self._network.update_fc(self._total_classes)        ### increment + init_classes
